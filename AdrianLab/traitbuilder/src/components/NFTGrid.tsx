...
import { NFT } from '../hooks/useUserNFTs';
        ...
        >
          ...
          <div className="aspect-square bg-gradient-to-br from-purple-500 to-blue-600 relative">
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-3">
              <h3 className="text-white font-semibold text-sm">{nft.name}</h3>
              <div className="text-gray-300 text-xs mt-1">
                <p>Gen: {nft.attributes.generation}</p>
                {nft.attributes.mutation && (
                  <p>Mutation: {nft.attributes.mutation}</p>
                )}
                {nft.attributes.skinName && (
                  <p>Skin: {nft.attributes.skinName}</p>
                )}
              </div>
            </div>
          </div>
