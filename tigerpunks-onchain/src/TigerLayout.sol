// SPDX-License-Identifier: MIT
pragma solidity >=0.8.17;

/// AUTO-GENERATED by script/gen_data.py — DO NOT EDIT.
/// 0-based flat-blob trait bases & counts (z-order), tiger-variant bases, tiger-punk test.
library TigerLayout {
    uint256 internal constant N_MODE = 5;
    uint256 internal constant N_PUNK = 8;
    uint256 internal constant P_PUNK = 0;
    uint256 internal constant P_TOP   = 8;    uint256 internal constant N_TOP   = 18;
    uint256 internal constant P_BEARD = 26;  uint256 internal constant P_BEARD_T = 36; uint256 internal constant N_BEARD = 10;
    uint256 internal constant P_HAIR  = 46;   uint256 internal constant P_HAIR_T  = 76;  uint256 internal constant N_HAIR  = 30;
    uint256 internal constant P_HAT   = 106;    uint256 internal constant P_HAT_T   = 133;   uint256 internal constant N_HAT   = 27;
    uint256 internal constant P_MOUTH = 160;  uint256 internal constant N_MOUTH = 8;
    uint256 internal constant P_EYE   = 168;    uint256 internal constant P_EYE_T   = 196;   uint256 internal constant N_EYE   = 28;
    uint256 internal constant P_MISC  = 224;   uint256 internal constant P_MISC_T  = 236;  uint256 internal constant N_MISC  = 12;

    function isTigerPunk(uint8 p) internal pure returns (bool) { return p == 4 || p == 5 || p == 7; }
}
