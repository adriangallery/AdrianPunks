...
async function initDB() {
  ...
  initPromise = (async () => {
    try {
      ...
      isInitialized = false;
      initPromise = null; // Permitir retry
      return false;
    }
  })();
...
/**
 * Esperar a que la Database esté lista
 * Útil para asegurar que la conexión esté establecida antes de hacer queries
...
async function waitForReady(timeout = 10000) {
  ...
  if (initPromise) {
    try {
      ...
      if (error.message === 'Timeout') {
        console.error('❌ Timeout esperando Database');
        return false;
      }
      throw error;
    }
  ...
  // Si no hay inicialización en progreso, iniciar una
  return await initDB();
}
...
if (typeof window !== 'undefined') {
  ...
  console.log('   Uso: await window.Database.waitForReady() antes de queries');
}
